...
async def ban_user(bot: Bot, group_id, user_id, time):
...
async def select_dirty(sql):
    conn = await asyncpg.connect(user=config["user"], password=config["password"], database=config["database"],
                                 host=config["host"])
    user_count = await conn.fetchrow(sql)
    await conn.close()
        ...
        if word in content:
            ...
            if check_gpName in group_name:  # 这里要做权限隔离  不要所有都检测
                ...
                try:
                    ...
                    """.format(sender_id, group_id)
                    count = await select_dirty(user_sql)
                    print("共有%s条数据" % count)
                    if is_delete == True:
