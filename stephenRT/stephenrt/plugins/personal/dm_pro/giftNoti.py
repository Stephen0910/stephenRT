...
async def gift_push():
    bot = get_bot()
    sql = "SELECT nn, room_user, num, gfn, single_price, timestamp FROM dm where single_price > {0} and timestamp > {1} ORDER BY timestamp DESC".format(single_money, init_time[-1])
    gifts = await get_presents(sql=sql)
    if gifts:
        init_time.append(gifts[0][-1])
        msg = ""
        for gift in gifts:
            msg += "{0} 送给 {1} {2}个 {3} ￥{4}/个 \n".format(gift[0], gift[1], gift[2], gift[3], gift[4])
        print(msg)
